...
export let options = {
  thresholds: {
    ...
    "http_reqs{request:commit_post}": [`count>=${rpsThresholds['count']}`],
  },
  stages: stages,
  rps: rps,
  setupTimeout: '60s'
...
export function createCommit(projectId, action) {
  let params = { headers: { "Accept": "application/json", "PRIVATE-TOKEN": `${__ENV.ACCESS_TOKEN}`, 'Content-Type': 'application/json' }, redirects: 0, tags: { request: 'commit_post' } };
