...
```json
{
  ...
  "git_push_data": {
    "export_file_path": "./push_data/gpt-bot_performance_export.tar.gz",
    "branch_current_head_sha": "1ff9883ebc8c0f0be01f919b5f45924dec456ed0",
    "branch_new_head_sha": "85a0dbc410185db5a051a08caf74239fafe59aaa",
    "branch_name": "gy-nfs-update"
  }
}
```
...
* Details for each project that the tests should target along with it's data. You should aim to have each of these details present here and in the target environment otherwise the specific tests that require them will be skipped automatically:
    ...
    * `mr_commits_iid` - The [iid] of a merge request available in the project that has a large number of commits. The size of the MR should be tuned to your environment's requirements.
    * `mr_discussions_iid` - The [iid] of a merge request available in the project that has a large number of discussions / comments. The size of the MR discussions should be tuned to your environment's requirements.
    * `user` - The name of a valid user for testing related endpoints.   
* Git push data that will be used for `git_push` test. No need to change anything if you are using the default `gpt-bot_performance_export.tar.gz` from the envirionment file. To test a custom project or learn more about git push test, please refer to [`Git Push test documentation`](test_docs/git_push.md). The size of the project and commits should be tuned to your environment's requirements.
    * `export_file_path` - Absolute path to the project export file that will be used to import the data to a new project.
...
[iid]: https://docs.gitlab.com/ee/api/#id-vs-iid
