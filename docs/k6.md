...
Options:
  ...
  -u, --upload-results     Upload test results to a InfluxDB server. Requires K6_INFLUXDB_URL Environment Variable to be set.
...
the project(s) to be tested and have API and read_repository permissions. (Default: nil)
  K6_INFLUXDB_URL             URL for a InfluxDB server that k6 will upload results to (creates database named k6db). (Default: nil)
