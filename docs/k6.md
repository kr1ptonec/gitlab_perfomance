...
The [Environment Config File](../k6/config/environments) configures details about the environment as well as how tests should be run against it. The file should already be prepared as part of the [Preparing the Environment](environment_prep.md##preparing-the-environment-file) steps. Refer back to that section in the docs if this file and the target GitLab environment isn't already prepared.

### Tests

The [k6 test files](https://docs.k6.io/docs/running-k6#section-executing-local-scripts). Each file contains a test to run against the environment along with any extra config such as setting thresholds.
...
Test files are native [k6 test scripts](https://docs.k6.io/docs/running-k6#section-executing-local-scripts) and all valid k6 features and options can be used here.
...
The recommended way to run the Tool is with our Docker image, [gitlab/gitlab-performance-tool](https://hub.docker.com/r/gitlab/gitlab-performance-tool) (alternative mirror: [registry.gitlab.com/gitlab-org/quality/performance/gitlab-performance-tool](https://gitlab.com/gitlab-org/quality/performance/container_registry)), which will be regularly updated with new features and tests. It can also be used in airgapped environments.
...
```txt
GitLab Performance Tool (GPT) v2.1.0 - Performance test runner for GitLab environments based on k6
...
Options:
  ...
  -o, --options=<s>         Deprecated option. Was previously used to pass in an Options Config file. If passed GPT will attempt to translate into scenario. Refer to docs for more info...
...
have all permissions set. (Default: nil)
  GPT_DEBUG                Shows debug output when set to true. (Default: nil)

Examples:
  Run all Tests against the 10k Environment:
    docker run -it gitlab/gitlab-performance-tool --environment 10k.json
  Run all API Tests against the 10k Environment:
    docker run -it gitlab/gitlab-performance-tool --environment 10k.json --tests api
  Run a specific Test against the 10k Environment:
    docker run -it gitlab/gitlab-performance-tool --environment 10k.json --tests api_v4_groups_projects.js
```
...
```sh
docker run -it -e ACCESS_TOKEN=<TOKEN> -v <HOST CONFIG FOLDER>:/config -v <HOST TESTS FOLDER>:/tests -v <HOST RESULTS FOLDER>:/results gitlab/gitlab-performance-tool --environment <ENV FILE NAME>.json
```
...
Next you would need to add any of your custom Environment and Test files to their respective directories. From the tool's root folder this would be `k6/config/environments` and `k6/tests` respectively.
...
Examples:
  Run all Tests against the 10k Environment:
    ./bin/run-k6 --environment 10k.json
  Run all API Tests against the 10k Environment:
    ./bin/run-k6 --environment 10k.json --tests api
  Run a specific Test against the 10k Environment:
    ./bin/run-k6 --environment 10k.json --tests api_v4_groups_projects.js
```
...
* Test Config:                RPS: 200, Duration: 60s
* Date:                       2020-06-09
