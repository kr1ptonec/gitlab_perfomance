...
Options:
  ...
  -h, --http-debug         Enable `k6` HTTP debug logs in output
...
Examples:
  Running all Tests with the 60s_200rps Options file against the 10k Environment:
    ./run-k6 --environment ./environments/10k.json --options ./options/60s_200rps.json
  Run a specific Test with the 20s_2rps Options file against the onprem Environment:
    ./run-k6 --environment ./environments/onprem.json --options ./options/20s_2rps.json --tests ./tests/api/api_v4_groups_projects.js
```
