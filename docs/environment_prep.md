...
Before running the GitLab Performance Tool, it requires the target GitLab environment to be prepared and seeded with specific test data.

This environment preparation is detailed on this page broken down into 3 parts: [Creating a User](#creating-a-user), [Generating an Access Token](#generating-an-access-token) and [Setting up test data with the GPT Data Generator](#setting-up-test-data-with-the-gpt-data-generator).
...
The first part of preparation you'll need to do is to create a user on the target environment. This user will be used for the seeding of test data and for running the performance tests themselves with GPT.

Please note the following requirements for the User:
...
We recommend a new user is created for this process that can then just be deleted cleanly after. On test setups the main root user can be used.

## Generating an Access Token

Next you will need to generate a GitLab Personal Access Token for authentication purposes for the User you created in the last section. This token is to be used with both the GPT Data Generator and GPT itself.

[The official GitLab docs detail how to create this token](https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html#creating-a-personal-access-token). Both of the tools (GPT and GPT Data Generator) require that the token is generated by an admin user and that it has all permissions.

Remember to keep this Access Token to use with GPT after.

## Setting up test data with the GPT Data Generator

With User and Access token created you should be ready to now seed data with the [GPT Data Generator](https://gitlab.com/gitlab-org/quality/performance/blob/master/bin/generate-gpt-data) tool. This tool has been designed to set up all the required test data that GPT itself requires.
...
To help visualize this here is a diagram of how the data will look:
...
Performing this setup is broken down into 2 sections:
...
For a new environment the following settings above will need to be changed accordingly: `name`, `url`, `config`, `latency`, `storage_nodes`

### Running the GPT Data Generator tool

When the environment config file is in place the [GPT Data Generator](../bin/generate-gpt-data) tool can now be run to seed the data into the environment.

#### Docker (Recommended)

The recommended way to run the GPT Data Generator is with our Docker image, [registry.gitlab.com/gitlab-org/quality/performance/gpt-data-generator](https://gitlab.com/gitlab-org/quality/performance/container_registry). It can also be used in [airgapped environments](#airgapped-environments).

The full options for running the tool can be seen by getting the help output by running `docker run -it registry.gitlab.com/gitlab-org/quality/performance/gpt-data-generator --help`:
...
Environment Variables:
  ACCESS_TOKEN             A valid GitLab Personal Access Token for the specified environment. The token should have admin access for the ability to create and import projects. (Default: nil)
...
The GitLab Performance Tool provides the ability to run performance tests against a target GitLab environment with custom Vertical Data as an advanced use case.

**Note:** Testing done against custom large projects should be in addition to tests with the [default opinionated data](#setting-up-test-data-with-the-gpt-data-generator) so that baseline analysis can still be done.

For this use case you will need to do the following:

1. [Configure Project Config file](#configure-project-config-file)
1. [Prepare Custom Test Data using the GPT Data Generator](#prepare-custom-test-data-using-the-gpt-data-generator)
1. [Set up Environment Config file and Run the GPT](#set-up-environment-config-file-and-run-the-gpt)
...
##### Prepare Custom Test Data using the GPT Data Generator

If the project you wish to test against isn't already available on the target environment then, to begin with, you will need to have the project's [export file](https://docs.gitlab.com/ee/user/project/settings/import_export.html#exporting-a-project-and-its-data) and [Environment Config file](#preparing-the-environment-config-file) prepared. With these prepared you can then generate your test data as follows:
...
* By default it will import the project into the same subgroup as the default data, `gpt/large_projects`. This is fine to do and even encouraged and GPT can still test either or of the projects.
  * If you wish to import into a different subgroup this can be done by passing the `--vert-group <GROUP NAME>` param on the above command.

##### Set up Environment Config file and Run the GPT

When the Project Config file is in place, specify its name in `large_project` setting in the Environment Config file if you are using the GPT [Docker image](k6.md##docker-recommended).

If you are using the GPT [natively on Linux](k6.md#linux), specify the full path of the Project Config file in `large_project` setting in the Environment Config and run the GPT as usual.
